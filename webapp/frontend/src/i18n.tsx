...
const DICT: Record<string, { en: string; zh: string }> = {
  ...
  'intro.subtitleBessCalc': {
    en: 'SA revenue cases combine SOC-constrained energy dispatch with observed DUID-level FCAS enablement.',
    zh: 'SA 收益情景结合带 SOC 约束的能量调度与 DUID 级 FCAS 实际启用量。',
  },
  'intro.titleFutures':  { en: 'Electricity Futures',                      zh: '电力期货' },
  'intro.subtitleFutures': {
    en: 'ASX Energy forward curves for hedging NEM regional price exposure',
    zh: '通过 ASX Energy 远期曲线管理 NEM 区域电价风险',
  },
  ...
  'fcasFc.title':        { en: 'FCAS · history + forecast · {0}',          zh: 'FCAS · 历史 + 预测 · {0}' },
  'fcasFc.hint':         { en: 'Actual dispatch + AEMO P5MIN/PREDISPATCH · $/MW/h', zh: '真实调度 + AEMO P5MIN/PREDISPATCH · $/MW/h' },
  'fcasFc.issued':       { en: 'issued',                                   zh: '发布于' },
  'fcasFc.power':        { en: 'Power',                                    zh: '功率' },
  'fcasFc.availability': { en: 'Available',                                zh: '可用' },
  'fcasFc.loading':      { en: 'Loading FCAS forecast…',                   zh: '加载 FCAS 预测…' },
  'fcasFc.error':        { en: 'FCAS forecast unavailable.',               zh: 'FCAS 预测暂不可用。' },
  'fcasFc.empty':        { en: 'No FCAS forecast rows in this window yet.', zh: '该窗口暂无 FCAS 预测数据。' },
  'fcasFc.recommendation': { en: 'Recommendation',                         zh: '推荐' },
  'fcasFc.recMsg':       { en: 'Prioritise {0} availability in the next forecast window.', zh: '下一预测窗口优先保留 {0} 可用容量。' },
  'fcasFc.noRecommendation': { en: 'No eligible FCAS market yet.',          zh: '暂未发现合适的 FCAS 市场。' },
  'fcasFc.windowRevenue': { en: 'Window revenue',                          zh: '窗口收益' },
  'fcasFc.avg':          { en: 'Avg',                                      zh: '均值' },
  'fcasFc.peak':         { en: 'Peak',                                     zh: '峰值' },
  'fcasFc.source':       { en: 'Source',                                   zh: '来源' },
  'fcasFc.curveTitle':   { en: 'FCAS Price Comparison',                   zh: 'FCAS 价格对比' },
  'fcasFc.actual':       { en: 'actual',                                   zh: '真实' },
  'fcasFc.forecast':     { en: 'forecast',                                 zh: '预测' },
  'fcasFc.rev':          { en: 'Rev',                                      zh: '收益' },
  'fcasFc.best':         { en: 'Best',                                     zh: '最佳' },
  'fcasFc.interval':     { en: 'Interval',                                 zh: '时段' },
  ...
  'forecast.skillNote':   { en: 'All models are scored on the SAME common intervals (fair compare). Skill = 1 − model RMSE / AEMO RMSE; positive means it beats AEMO. Bias <0 means it under-forecasts. NEM predispatch only reaches end-of-next-trading-day, so AEMO\'s coverage is partial.', zh: '所有模型在同一批共同时点上打分(口径一致)。Skill = 1 − 模型 RMSE / AEMO RMSE,为正即跑赢 AEMO;偏差 <0 表示系统性低估。NEM predispatch 只预测到次日凌晨,故 AEMO 覆盖是部分的。' },
  'forecast.commonPts':   { en: 'common intervals', zh: '个共同时点对比' },
  'forecast.coverage':    { en: 'cover', zh: '覆盖' },
  'forecast.coverageHint':{ en: 'Intervals this model forecast (with an actual) in the window — coverage, not the scored common set.', zh: '该模型在窗口内有预测(且有实际值)的时点数=覆盖;打分用的是共同时点。' },
  ...
  'bc.bt.energy':        { en: 'Energy cash settlement',                    zh: '能量套利现金收入' },
  'bc.bt.fcas':          { en: 'FCAS observed benchmark',                   zh: 'FCAS 实际基准' },
  'bc.bt.combined':      { en: 'Scenario total',                            zh: '情景合计' },
  'bc.bt.scenarioPerYear': { en: '/yr theoretical upper',                   zh: '/年（理论上限）' },
  // Price forecast
  'fc.title':            { en: 'P5MIN Forecast',                            zh: 'P5MIN预测' },
  'fc.band':             { en: 'Uncertainty band (±1σ)',                    zh: '不确定范围 (±1σ)' },
  'bc.bt.impliedSpread': { en: 'Captured cash margin',                     zh: '套利现金毛利' },
  'bc.bt.impliedPerMw':  { en: 'Price exposure per MW',                    zh: '单位 MW 价格暴露' },
  'bc.bt.capture':       { en: 'Capture',                                  zh: '捕获率' },
  'bc.bt.marketMargin':  { en: 'Captured market',                          zh: '市场收益' },
  'bc.bt.degCost':       { en: 'degradation',                              zh: '降解成本' },
  'bc.bt.economicNet':   { en: 'Economic net',                             zh: '经济净值' },
  'bc.bt.degHurdle':     { en: 'cycling hurdle',                           zh: '循环机会成本' },
  'bc.bt.assumedEnabled': { en: 'assumed enabled',                         zh: '假设启用比例' },
  'bc.bt.fcasWarning':   {
    en: 'FCAS is not realised asset revenue. It sums all 10 regional prices and assumes the selected share of nameplate MW is enabled in every service. Actual revenue needs DUID enabled MW, offers, trapeziums, SOC and NEMDE co-optimisation.',
    zh: 'FCAS 并非电站已实现收益。本情景将 10 个区域 FCAS 价格相加，并假设各服务均启用设定比例的额定功率。准确收益必须接入 DUID 启用 MW、报价梯形、SOC 及 NEMDE 联合优化数据。',
  },
  'bc.bt.bestDay':       { en: 'Best day',                                 zh: '最佳单日' },
  'bc.bt.monthly':       { en: 'Monthly energy upper bound',               zh: '逐月能量套利理论上限' },
  ...
  'bc.bt.methodology':   {
    en: 'Energy upper bound: daily chronological perfect-foresight optimisation with explicit SOC, power, cyclic end-SOC, RTE, project MLF, auxiliary load, cycling hurdle and cycle cap. It is an audit ceiling, not the finance-model base case.',
    zh: '能量上限：逐日完全预知优化，明确约束 SOC、功率、日终 SOC、RTE、项目 MLF、自用电、循环机会成本及循环上限。它仅用于审查收益天花板，不作为财务模型基准值。',
  },
  ...
  'bc.wf.afterCapture':  { en: 'After capture eff.',                        zh: '扣捕获率后' },
  'bc.wf.afterDegradation': { en: 'Net after degradation',                  zh: '扣降解后的净收益' },
  'bc.wf.hint':          { en: '',                                          zh: '' },
  ...
  'bc.in.arbSpread':     { en: 'Captured energy cash margin',              zh: '套利现金毛利' },
  'bc.in.fcasRev':       { en: 'Observed FCAS benchmark (yr1)',            zh: 'FCAS 实际基准(首年)' },
  'bc.in.fcasDecline':   { en: 'FCAS decline scenario',                    zh: 'FCAS 年衰减情景' },
  // Observed DUID benchmark panel
  'bc.bm.title':         { en: 'SA observed operating revenue',            zh: 'SA 电池实际运营收益' },
  'bc.bm.loading':       { en: 'Scaling SA BESS operating actuals to the target…', zh: '正在将 SA 电池实际运营数据映射到目标项目…' },
  'bc.bm.waiting':       { en: 'Benchmark loads after the asset spec is set.', zh: '设置项目参数后加载实际基准。' },
  'bc.bm.unavailable':   { en: 'Observed benchmark unavailable',           zh: '实际基准暂不可用' },
  'bc.bm.noRegionalActuals': { en: 'No DUID actual benchmark for this region', zh: '该区域暂没有 DUID 实际收益基准' },
  'bc.bm.saOnly':        { en: 'The verified comparator dataset currently covers SA only.', zh: '当前已核验的可比项目数据仅覆盖 SA。' },
  'bc.bm.recomputing':   { en: 'recomputing…',                             zh: '重新测算中…' },
  'bc.bm.operational':   { en: 'established comparables',                  zh: '个成熟可比项目' },
  'bc.bm.case':          { en: 'Case',                                     zh: '情景' },
  'bc.bm.energy':        { en: 'Energy cash',                              zh: '套利现金' },
  'bc.bm.total':         { en: 'Operating cash',                           zh: '运营现金' },
  'bc.bm.conservative':  { en: 'Conservative',                             zh: '保守' },
  'bc.bm.base':          { en: 'Base',                                     zh: '基准' },
  'bc.bm.upside':        { en: 'Upside',                                   zh: '上行' },
  'bc.bm.mlfWarning':    { en: 'MLF is still 1.000. Enter the project connection-point/DUID MLF before relying on the result.', zh: 'MLF 仍为 1.000。正式采用测算前，请输入项目接入点/DUID 对应的 MLF。' },
  'bc.bm.showComparables': { en: 'Show comparator DUIDs',                  zh: '查看可比项目 DUID' },
  'bc.bm.commissioningExcluded': { en: 'are shown in the source data but excluded from quartiles as commissioning-year assets.', zh: '仍展示在原始数据中，但因处于投运初期而未纳入分位数。' },
  'bc.bm.energyMethod':  { en: 'actual DUID cash margin per discharged MWh × realised cycles × target capacity', zh: '实际 DUID 每放电 MWh 现金毛利 × 实际循环 × 目标容量' },
  'bc.bm.fcasMethod':    { en: 'actual enabled MW × matching service price / 12', zh: '实际启用 MW × 对应服务价格 ÷ 12' },
  'bc.bm.baseBridge':    { en: 'Base energy cash − degradation reserve = energy operating contribution', zh: '基准套利现金 − 退化成本 = 套利运营贡献' },
  'bc.bm.applyBase':     { en: 'Apply observed operating base case',       zh: '应用实际运营基准情景' },
  'bc.bm.limitations':   { en: 'AEMO DISPATCHLOAD INITIALMW proxy; FCAS excludes FPP, contracts and participant fees. Energy + FCAS is an empirical comparator combination, not interval-level target co-optimisation. The realised equivalent-cycle distribution for the available 0.4–2.4h SA basket is extrapolated to a 4h target.', zh: '套利采用 AEMO DISPATCHLOAD 的 INITIALMW 代理；FCAS 不含 FPP、合约及参与者费用。套利与 FCAS 是可比项目经验组合，并非目标项目逐时联合优化；4h 项目的循环强度由现有 0.4–2.4h SA 样本外推。' },
  ...
  'bc.rev.note':         {
    en: 'Energy follows battery capacity fade; FCAS follows the user decline scenario (default 0%).',
    zh: '能量收益随电池容量衰减；FCAS 按用户设定的年衰减情景变化（默认 0%）。',
  },
